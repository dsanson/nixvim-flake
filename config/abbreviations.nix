...

{
  extraConfigVim = ''
    ...
    " some more typos
    iab <buffer> mabye maybe
    iab <buffer> homogenous homogeneous
    iab <buffer> possiblity possibility

    " arabic
    iab <buffer> qadiyya qaḍiyya
    iab <buffer> hikaya ḥikāya
    iab <buffer> kalam kalām
    iab <buffer> Kalam Kalām
    iab <buffer> hadith ḥadīth
    iab <buffer> insha inshāʾ
    iab <buffer> fahwa faḥwā
    iab <buffer> mana maʿnā
    iab <buffer> maani maʿānī
    iab <buffer> madmun maḍmūn
    iab <buffer> hukman ḥukman
    iab <buffer> haqiqiya ḥaqīqiya
    iab <buffer> lafz lafẓ
    iab <buffer> kharjiyya khārjiyya
    iab <buffer> Dawani Dawānī
    iab <buffer> Jalal Jalāl
    iab <buffer> Amidi Āmidī
    iab <buffer> Hilli Ḥillī
    iab <buffer> Dashtaki Dashtakī
    iab <buffer> Astarabadi Astarābādī
    iab <buffer> Tusi Ṭūsī
    iab <buffer> Abhari Abharī
    iab <buffer> Taftazani Taftāzānī
    iab <buffer> Katibi Kātibī
    iab <buffer> Samarqandi Samarqandī
    iab <buffer> Jurjani Jurjānī
    iab <buffer> Kammuna Kammūna
    iab <buffer> Qadi Qādī
    iab <buffer> Jabbar Jabbār
    iab <buffer> Sakaki Sakākī
    iab <buffer> Mutazilah Muʿtazilah
    iab <buffer> Quran Qurʾān
    iab <buffer> Yahya Yaḥyā
    iab <buffer> Adi ʿAdī
    iab <buffer> Sirafi Sirāfī
    iab <buffer> Tawhidi Tawḥīdī
    iab <buffer> Farabi Fārābī
    iab <buffer> Abu Abū
    iab <buffer> Matta Mattā
    iab <buffer> Baghdadi Baghdādī
    iab <buffer> Basri Baṣrī
    iab <buffer> Ashari Ashʿarī
    iab <buffer> Mutazila Muʿtazila
    iab <buffer> Mutazala Muʿtazila
    iab <buffer> Mutazili Muʿtazilī
    iab <buffer> Mutazali Muʿtazilī
    iab <buffer> Mutakallimun Mutakallimūn
    iab <buffer> Mutakalimun Mutakallimūn
    iab <buffer> Baqillani Bāqillānī
    iab <buffer> Muhammad Muḥammad
    iab <buffer> Muhammed Muḥammad
    iab <buffer> Shiite Shīʿite
    iab <buffer> Shiism Shīʿism
    iab <buffer> Shia Shīʿa
    iab <buffer> tawhid tawḥīd
    iab <buffer> Qushji Qūshjī
    iab <buffer> anhu ʿanhu
    iab <buffer> Tajrid Tajrīd
    iab <buffer> Razi Rāzī
    iab <buffer> Iji Ījī
    iab <buffer> Ghazali Ghazālī
    iab <buffer> Sina Sīnā
    iab <buffer> Nazzam Naẓẓām
    iab <buffer> tasdiq taṣdīq
